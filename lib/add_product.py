...
class AddProductTopWindow(ThemeEngine):
    ...
    def __init__(self, master):
        ...
        self.clear_btn = tk.Button(apw_frame2, image=self.clear_btn_img, bd=0,
                           bg=self.button_bg, activebackground=self.button_bg,
                           command=self.clear_entries)
        self.clear_btn.place(x=47, y=200)

        # Import Products from CSV Button
        self.import_csv_img = tk.PhotoImage(file='images/import_button.png')
        self.import_csv_btn = tk.Button(apw_frame2, image=self.import_csv_img, bd=0,
                           bg=self.button_bg, activebackground=self.button_bg,
                           command=self.import_from_csv)
        self.import_csv_btn.place(x=47, y=300)

        # Close Button
        self.close_btn_img = tk.PhotoImage(file='images/close_button.png')
        self.close_btn = tk.Button(apw_frame2, image=self.close_btn_img, bd=0,
                           bg=self.button_bg, activebackground=self.button_bg,
                           command=self.close_window)
        self.close_btn.place(x=47, y=400)
    ##============================ Other METHODS ===========================================##
        ...
        if file_path:
            try:
                with open(file_path, newline='', encoding='utf-8') as csvfile:
                    ...
                    for row in reader:
                        if len(row) == 5:
                            name, category, subcategory, price, quantity = row
                            # Assuming you have a method `add_product` to add products to your database
                            db.add_product(name, category, subcategory, float(price), int(quantity))
                    tk.messagebox.showinfo("Success", "Products imported successfully!")
            except Exception as e:
